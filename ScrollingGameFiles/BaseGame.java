...
public class BaseGame extends ScrollingGame {
	...
	private final String USER_IMG = "user_base.gif";    // specify user image file
    ...
    */
 	...
 	/*
 	Checks the randomly generated last column and second last column to ensure that at least one of the rows doesn't have two consecutive avoid objects,
 	in the two columns, to ensure that the user always has a path to avoid hitting unhealthy objects and losing score. If no such path exists, creates a get object and 
 	...
 	*/
 	public void path_clear_overall() {
 		...
 		grid.setCellImage(new Location(H_DIM-1, W_DIM-1), GET_IMG);
 		grid.setCellImage(new Location(H_DIM-2, W_DIM-1), null);
 	}
 	
 	/*
 	Helper method to path_clear_overall(): Returns true if at least one of the rows doesn't have avoid objects,
 	in the two columns.
 	*/
 	
 	public boolean condition(int i) {
 		if ((!AVOID_IMG.equals(grid.getCellImage(new Location(i, W_DIM-1))) && !USER_IMG.equals(grid.getCellImage(new Location(i, W_DIM-1))))) {
 			if ((!AVOID_IMG.equals(grid.getCellImage(new Location(i, W_DIM-2))) && !USER_IMG.equals(grid.getCellImage(new Location(i, W_DIM-2))))){
 				return true;
 		}
 		}
 		return false;
